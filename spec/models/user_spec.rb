...
describe User do
  describe '#create' do
    ...
    # 6. nicknameが1文字であれば登録できないこと
    it "is invalid with a nickname that has  7 characters " do
      user = build(:user, nickname: "a")
      user.valid?
      expect(user.errors[:nickname]).to include("は2文字以上で入力してください")
    end

    # 7. nicknameが2文字以上では登録できること
    it "is valid with a nickname that has more than 3 characters " do
